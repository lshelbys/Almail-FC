const translations = {
    en: {
        // Promo Banner
        'promo-text': 'Professional Amateur Football Club — Established 2021',
        'banner-contact': 'Contact',

        // Header Nav
        'nav-news': 'NEWS',
        'nav-squad': 'SQUAD',
        'nav-matches': 'MATCHES',
        'nav-tickets': 'TICKETS',
        'nav-shop': 'SHOP',
        'nav-club': 'CLUB',

        // Secondary Nav
        'sec-latest': 'Latest',
        'sec-results': 'Results',
        'sec-schedule': 'Schedule',
        'sec-stadium': 'Stadium',

        // Hero
        'hero-title': 'ALMAIL SPORTS CLUB',
        'hero-subtitle': 'Professional Amateur Football Club',
        'hero-btn-results': 'VIEW RESULTS',
        'hero-btn-squad': 'OUR SQUAD',

        // News Section
        'news-title': 'LATEST NEWS',
        'news-category': 'CLUB NEWS',
        'news-article-title': 'Almail SC Renews Amateur Club Status for 2025',
        'news-article-desc': 'Almail Sports Club is proud to announce the renewal of its amateur club status for the 2025 season, reaffirming our commitment to grassroots football and developing talent in the community.',
        'news-read-more': 'READ FULL ARTICLE',
        'news-date': '17 JUN 2026',

        // Matches Section
        'matches-title': 'MATCHES',
        'season-2025': '2025',
        'season-2024': '2024',
        'season-2023': '2023',
        'season-2022': '2022',
        'competition-dumankaya': 'DUMANKAYA CUP',
        'match-final': 'FINAL',
        'team-almail': 'ALMAIL SC',
        'team-shabab': 'SHABAB AL HURA',
        'home': 'Home',
        'away': 'Away',
        'ft': 'FT',
        'upcoming': 'UPCOMING',
        'match-details': 'MATCH DETAILS',
        'matches-coming-soon': 'Matches Coming Soon',
        'season-records': 'Match records for the',
        'season-coming': 'season are coming soon.',

        // Match Modal
        'modal-details': 'DETAILS',
        'modal-goals': 'GOALS',
        'modal-lineup': 'LINE-UP',
        'competition': 'Competition',
        'round': 'Round',
        'date': 'Date',
        'kickoff': 'Kick-off',
        'venue': 'Venue',
        'result': 'Result',
        'almail-win': 'Almail SC WIN',
        'goal-timeline': 'Goal timeline coming soon.',
        'lineup-details': 'Line-up details coming soon.',

        // Squad Section
        'squad-title': 'MEN\'S FIRST TEAM',
        'squad-coming': 'Squad Coming Soon',
        'player-profiles': 'Player profiles for the Almail Sports Club squad will be published soon. Check back for full squad details.',

        // Tickets Section
        'tickets-title': 'TICKETS & MEMBERSHIP',
        'tickets-coming': 'Tickets Coming Soon',
        'tickets-desc': 'Ticketing services will be available soon. Stay tuned for updates on how to get your match tickets.',

        // Shop Section
        'shop-title': 'OFFICIAL SHOP',
        'shop-coming': 'Official Shop Coming Soon',
        'shop-desc': 'Our official merchandise shop is launching soon. Check back for exclusive Almail SC gear and apparel.',

        // Club Info
        'club-title': 'CLUB INFORMATION',
        'stadium': 'STADIUM',
        'stadium-desc': 'Bizarri\'s Stadium, proudly sponsored by Bizarri, is the home ground of Almail Sports Club with world-class facilities.',
        'learn-more': 'LEARN MORE',
        'history': 'HISTORY',
        'history-desc': 'The full history of Almail Sports Club is coming soon. Discover the journey of the club from its founding to today.',
        'coming-soon': 'COMING SOON',
        'player-form': 'PLAYER FORM',
        'player-form-desc': 'Interested in joining Almail SC? Get in touch with the club directly. Please provide: name, position, date of birth, country, height, and a photo.',
        'email-us': 'EMAIL US',
        'whatsapp': 'WHATSAPP',
        'contact-us': 'CONTACT US',
        'contact': 'CONTACT',
        'contact-desc': 'Get in touch with us for general enquiries about the club, partnerships, or supporter information.',

        // Footer
        'footer-tagline': 'Professional Amateur Football',
        'footer-club': 'CLUB',
        'footer-about': 'About Us',
        'footer-contact': 'Contact',
        'footer-football': 'FOOTBALL',
        'footer-first-team': 'First Team',
        'footer-fixtures': 'Fixtures',
        'footer-follow-us': 'FOLLOW US',
        'footer-copyright': '© 2026 Almail Sports Club. All rights reserved. • Bizarri\'s Stadium',
    },
    ar: {
        // Promo Banner
        'promo-text': 'نادي رياضي هاوي احترافي — تأسس 2021',
        'banner-contact': 'اتصل بنا',

        // Header Nav
        'nav-news': 'أخبار',
        'nav-squad': 'الفريق',
        'nav-matches': 'المباريات',
        'nav-tickets': 'التذاكر',
        'nav-shop': 'المتجر',
        'nav-club': 'النادي',

        // Secondary Nav
        'sec-latest': 'الأحدث',
        'sec-results': 'النتائج',
        'sec-schedule': 'الجدول',
        'sec-stadium': 'الملعب',

        // Hero
        'hero-title': 'نادي المايل الرياضي',
        'hero-subtitle': 'نادي كرة قدم هاوي احترافي',
        'hero-btn-results': 'عرض النتائج',
        'hero-btn-squad': 'فريقنا',

        // News Section
        'news-title': 'أحدث الأخبار',
        'news-category': 'أخبار النادي',
        'news-article-title': 'نادي المايل ينجدد حالته كنادي هاوي لعام 2025',
        'news-article-desc': 'يفخر نادي المايل الرياضي بإعلان تجديد حالته كنادي هاوي للموسم 2025، مؤكداً التزامنا بكرة القدم الشعبية وتطوير المواهب في المجتمع.',
        'news-read-more': 'اقرأ المقال الكامل',
        'news-date': '17 يونيو 2026',

        // Matches Section
        'matches-title': 'المباريات',
        'season-2025': '2025',
        'season-2024': '2024',
        'season-2023': '2023',
        'season-2022': '2022',
        'competition-dumankaya': 'كأس دومانكايا',
        'match-final': 'النهائي',
        'team-almail': 'نادي المايل',
        'team-shabab': 'شباب الهرة',
        'home': 'الملعب الخاص',
        'away': 'الملعب الضيف',
        'ft': 'انتهت',
        'upcoming': 'قادمة',
        'match-details': 'تفاصيل المباراة',
        'matches-coming-soon': 'المباريات قريباً',
        'season-records': 'سجلات مباريات',
        'season-coming': 'قادمة قريباً.',

        // Match Modal
        'modal-details': 'التفاصيل',
        'modal-goals': 'الأهداف',
        'modal-lineup': 'التشكيلة',
        'competition': 'المسابقة',
        'round': 'الدور',
        'date': 'التاريخ',
        'kickoff': 'بدء المباراة',
        'venue': 'الملعب',
        'result': 'النتيجة',
        'almail-win': 'فوز نادي المايل',
        'goal-timeline': 'سجل الأهداف قادم قريباً.',
        'lineup-details': 'تفاصيل التشكيلة قادمة قريباً.',

        // Squad Section
        'squad-title': 'فريق الرجال الأول',
        'squad-coming': 'الفريق قريباً',
        'player-profiles': 'ستتم نشر ملفات لاعبي فريق نادي المايل الرياضي قريباً. عد للاطلاع على تفاصيل الفريق الكاملة.',

        // Tickets Section
        'tickets-title': 'التذاكر والعضوية',
        'tickets-coming': 'التذاكر قريباً',
        'tickets-desc': 'ستتاح خدمات التذاكر قريباً. ترقب آخر التحديثات حول كيفية الحصول على تذاكرك.',

        // Shop Section
        'shop-title': 'المتجر الرسمي',
        'shop-coming': 'المتجر قريباً',
        'shop-desc': 'سيتم إطلاق متجرنا الرسمي للبضائع قريباً. عد للاطلاع على منتجات نادي المايل الحصرية.',

        // Club Info
        'club-title': 'معلومات النادي',
        'stadium': 'الملعب',
        'stadium-desc': 'ملعب بيزاري، برعاية شركة بيزاري، هو ملعب نادي المايل الرياضي مع مرافق عالمية.',
        'learn-more': 'اقرأ المزيد',
        'history': 'التاريخ',
        'history-desc': 'سيأتي التاريخ الكامل لنادي المايل الرياضي قريباً. اكتشف رحلة النادي من تأسيسه حتى اليوم.',
        'coming-soon': 'قريباً',
        'player-form': 'نموذج اللاعب',
        'player-form-desc': 'هل أنت مهتم بالانضمام إلى نادي المايل؟ تواصل مع النادي مباشرة. يرجى تقديم: الاسم والمركز وتاريخ الميلاد والدولة والطول والصورة.',
        'email-us': 'أرسل بريداً',
        'whatsapp': 'واتساب',
        'contact-us': 'تواصل معنا',
        'contact': 'اتصل',
        'contact-desc': 'تواصل معنا للاستفسارات العامة عن النادي أو الشراكات أو معلومات المشجعين.',

        // Footer
        'footer-tagline': 'نادي كرة قدم هاوي احترافي',
        'footer-club': 'النادي',
        'footer-about': 'عن النادي',
        'footer-contact': 'اتصل',
        'footer-football': 'كرة القدم',
        'footer-first-team': 'الفريق الأول',
        'footer-fixtures': 'الجدول',
        'footer-follow-us': 'تابعنا',
        'footer-copyright': '© 2026 نادي المايل الرياضي. جميع الحقوق محفوظة. • ملعب بيزاري',
    }
};

let currentLanguage = 'en';

function setLanguage(lang) {
    currentLanguage = lang;
    localStorage.setItem('almailLanguage', lang);

    // Update all translatable elements
    document.querySelectorAll('[data-i18n]').forEach(el => {
        const key = el.getAttribute('data-i18n');
        if (translations[lang] && translations[lang][key]) {
            el.textContent = translations[lang][key];
        }
    });

    // Update HTML lang and dir attributes
    document.documentElement.lang = lang;
    document.documentElement.dir = lang === 'ar' ? 'rtl' : 'ltr';
    document.body.dir = lang === 'ar' ? 'rtl' : 'ltr';

    // Update language button states
    document.querySelectorAll('.lang-btn').forEach(btn => {
        btn.classList.toggle('active', btn.getAttribute('data-lang') === lang);
    });

    // Trigger any language-dependent features
    if (lang === 'ar') {
        document.body.classList.add('rtl-mode');
    } else {
        document.body.classList.remove('rtl-mode');
    }
}

// Initialize language on page load
document.addEventListener('DOMContentLoaded', () => {
    const savedLang = localStorage.getItem('almailLanguage') || 'en';
    setLanguage(savedLang);
});
